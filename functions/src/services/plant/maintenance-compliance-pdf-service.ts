...
export type MaintenanceCompliancePdfInput = {
  ...
  recentQualityLogs?: WaterQualityLogRecord[];
};
...
export function buildMaintenanceCompliancePdf(
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (input.recentQualityLogs?.length) {
        doc.moveDown(0.6);
        doc.fillColor("#111827").font("Helvetica-Bold").fontSize(12).text("Water quality log");
        doc.moveDown(0.3);
        doc.font("Helvetica").fontSize(9);
        for (const log of input.recentQualityLogs) {
          const passLabel =
            log.pass === true ? "pass" :
              log.pass === false ? "fail" :
                "—";
          doc.text(
            `${log.recordedAt.slice(0, 10)} · ${log.locationTag} · TDS ${log.tdsPpm} ppm · ${passLabel}`,
          );
          if (log.notes) doc.fillColor("#6b7280").text(`Notes: ${log.notes}`);
          doc.fillColor("#111827");
          doc.moveDown(0.15);
        }
      }
      ...
      doc
        .fontSize(8)
        .fillColor("#9ca3af")
        .text("Generated by Smart Refill · Plant maintenance compliance export", margin, doc.y, {
          width,
          align: "center",
        });
